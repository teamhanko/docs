...
However, if you're already using JavaScript/TypeScript for your backend, you can use [\@teamhanko/passkey-backend-sdk](https://www.npmjs.com/package/@teamhanko/passkey-backend-sdk), which do all of the below for you.
...
1. **Get your tenant ID**
    ...
    [👉 Explanation for tenant ID](faq#tenant-id-and-multitenancy)
...
2. **Add endpoints to start and finish passkey registration**
    ...
    Registering passkeys is a two-step process. First, let's add an endpoint to our backend. This example uses Express.js, but you can use whatever language and framework you prefer.
    ...
    **Frontend:**
    ...
    The process looks very, very similar for logging in:
...
3. **Add endpoints to start and finish logging in**
    ...
    **Frontend:**
    ...
    Note that for logging in, the server can also talk to the Passkey API directly, instead of going through your backend first. Whether you go with the server-first or client-first approach is up to preference. See [Client-First Login Flow](client-first-login-flow)
