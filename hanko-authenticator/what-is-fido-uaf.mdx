...
description: A high-level overview of the FIDO Universal Authentication Framework (UAF) protocol that powers the Hanko Authenticator.
---
...
FIDO UAF aimed at providing a generic framework for **passwordless** and **multi-factor** authentication, potentially leveraging the capabilities of a wide range of existing hardware devices (built-in or **bound** platform capabilities like fingerprint sensors, cameras, microphones, embedded TPM hardware) but also leaving open the possibility of using portable (also: **roaming**) devices not bound to any one specific device. This is the protocol the **Hanko Authenticator** and the **Hanko FIDO UAF Client** are built on.
