...
2.  Navigate to GitHub and click on your profile photo at the top right corner.
3.  Select 'Settings' at the bottom of the sidebar menu.
...
4.  Scroll down and select 'Developer Settings' in the left sidebar.
...
5.  Choose 'OAuth Apps' followed by 'New OAuth App'. This action opens the registration form. But before it, you need to retrieve the 'Redirect URL' from the Hanko cloud.
...
6.  Get back to OAuth app registration form, fill in the required details, and paste the copied redirect URL into the 'Application Callback URL' field. Finish by clicking 'Register application'.
...
7.  You'll now see an overview of OAuth app, note down the provided 'Client ID'. Also, generate a 'Client Secret' by selecting 'Generate a new client secret'. Copy these, as you'll need them in next step.
    <img
...
2.  In the 'Providers' section, select GitHub and enable its toggle switch.
