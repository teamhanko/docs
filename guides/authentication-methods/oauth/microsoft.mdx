...
2. On the main dashboard select `App registrations` under `Azure services`.

<Frame>
    <img
        src="/images/oauth-providers/microsoft/dashboard.png"
        alt="Select app registrations on dashboard screen"
        width={500}
    />
</Frame>

3. On the top left, click on `New registration`.
...
5. Under `Supported account types` choose the 3rd option i.e `Accounts in any organizational directory (Any Azure AD directory - Multitenant) and personal Microsoft accounts (e.g. Skype, Xbox)`.
    <Warning>
...
6. Under the `Redirect URI` section select `Web` as the platform.
7. Enter the `Redirect URL` you obtained in the [first step](#get-your-provider-redirect-url) as the value.
8. Click on `Register`.
...
<Tabs>
    ...
    <Tab title="Hanko Frontend SDK">
        ...
        Create a [`Hanko` client](https://docs.hanko.io/jsdoc/hanko-frontend-sdk/Hanko.html) instance and call  the
