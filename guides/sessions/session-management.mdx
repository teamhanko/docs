...
<Note>
    When using [Hanko Elements](/guides/hanko-elements/introduction) or the [Hanko Frontend SDK](/resources/sdks#sdks),
    the session token is read from the `X-Auth-Token` header and used to set a cookie (name: `hanko`) on the client-side.
    If you use neither Hanko Elements nor the SDK, you have to take care of storing the session token yourself.
</Note>
...
on one device but also has multiple sessions across other devices, the API's [Profile](/api-reference/flow/profile)
Flow allows listing and explicitly terminating the session on those devices. This feature is also reflected in the UI
provided by the Hanko Elements' [Profile Component](/guides/hanko-elements/profile-component).
...
1. Fetch the JWKS from the`.well-known/jwks.json` [endpoint](/api-reference/public/well-known/get-json-web-key-set).
...
<Accordion title="Third-party library JWK set support">
    ...
    , see [this guide](/quickstarts/backend/node#express) for an example of its
    ...
    JWK to verify the token (see [this](/quickstarts/backend/rust) guide for an example).
</Accordion>

### Validating stateful sessions

With stateful sessions you can instead use the `sessions/validate` [endpoint](/api-reference/public/session-management/validate-a-session-1)
...
[endpoint](http://localhost:3000/api-reference/public/user-management/log-out-the-current-user).
