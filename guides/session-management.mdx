---
sidebarTitle: Sessions and tokens
title: Sessions and tokens in Hanko
description: Learn how to manage Hanko authentication sessions, configure duration, validate tokens, limit concurrent sessions, customize JWT tokens, and handle revocation.
---
...
- `.User.CustomClaims`: The user's resolved [custom claims](/guides/user-data/custom-claims) (optional)
...
`.User.CustomClaims` provides access to a user's resolved [custom claims](/guides/user-data/custom-claims) - values
your enterprise or social connections have mapped onto claims you've declared for your project. Individual values
can be accessed by passing the claim name as an argument, the same way as `.User.Metadata.Public`/
`.User.Metadata.Unsafe` above.

Assume a user's resolved custom claims look like this:
...
Then you could access individual values, or embed the whole object directly:
...
<Warning>
    A claim's real type (number, boolean, array) is preserved only when the template value is nothing but a single,
    bare `.User.CustomClaims` reference like the examples above. Glue it to other text (e.g.
    `"ID: {{ .User.CustomClaims \"id\" }}"`) and normal Go template stringification applies instead, so a `number`
...
<ResponseField name="iss" type="string">
    Identifies the entity that issued the token. When using Hanko Cloud, `iss` will be your `API URL`.
</ResponseField>

<Warning>
    When using a custom domain with Hanko Cloud, the `iss` claim currently does not change and remains in the default format: `<project-id>.hanko.io`.
    This behavior is planned to change in a future release so that the configured custom domain will be used as the `iss` value.
</Warning>
