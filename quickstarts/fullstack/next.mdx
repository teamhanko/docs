...
Lets use the Hanko SDK to Get User and Session Data to log to the console.
...
For the Session Information we will use [hanko.validateSession()](https://teamhanko.github.io/hanko/jsdoc/hanko-frontend-sdk/index.html#getting-the-user-object)
    ...
    useEffect(() => setHanko(new Hanko(hankoApi)), []);
      useEffect(() => {
        ...
        hanko?.validateSession()
          .then((sessionStatus) => {
            console.log("Session status:", sessionStatus);// Log session Status
            console.log(sessionStatus.is_valid)
          })
      }, [hanko]);
