...
The Hanko client from `@teamhanko/hanko-elements` allows you to subscribe to specific [events](/guides/hanko-elements/using-frontend-sdk#events). For instance, the `onAuthFlowCompleted` event here triggers when a user successfully logs in. You can use this event to perform any desired action.
