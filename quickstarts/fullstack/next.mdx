...
We will create two custom hooks to fetch the current user's data and session data from `@teamhanko/hanko-elements`. The `useUserData` hook leverages the `hanko.user.getCurrent()` method to retrieve the currently logged-in user's data. 

On the other hand, the `useSessionData` hook uses `hanko.session.isValid()` and `hanko.session.get()` methods from Hanko to validate and fetch the current session data.
...
On the server side, you can extract the `userID` from the JWT. However, if you need to retrieve the complete user details, you'll have to utilize the [Hanko Admin API](https://docs.hanko.io/api-reference/admin/status/status-page).
...
<Info>
Hanko Admin API is a paid feature and must be enabled separately. You also need an API key `secret` to access the Hanko Admin API. You can generate one under the `Settings/API Keys` section of your project.
