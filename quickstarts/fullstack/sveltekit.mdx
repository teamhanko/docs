...
Both of these components use the Hanko client from [@teamhanko/hanko-elements](/guides/hanko-elements/introduction).
...
Here we subscribe to the `onSessionCreated` [event](/guides/hanko-elements/using-frontend-sdk#events), this triggers when a user successfully logs in. You can use these event to perform any desired action.
...
After setting up the HankoAuth lets Setup the `HankoProfile.jsx` file to create an interface where users can
manage their `Password` , `Email Addresses`, `Passkeys`, `Security Keys`.
...
Lets update the `dashboard` page to display some of the information from the user.
