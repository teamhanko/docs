...
After setting up the HankoAuth let's set up the `HankoProfile` component to create a interface where users can.\
...
Now let's set up the HTML and TS of the LogoutButton component.
...
Now to use this authentication guard you have to add it to your `app.routes.ts` file.
...
Lets update our dashboard page html and ts files.
