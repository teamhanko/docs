...
Once you've initialized your Solid app, installing hanko-elements provides you with access to the prebuilt components: `hanko-auth` and `hanko-profile`.
...
The `<hanko-auth>` web component adds a login interface to your app. Begin by importing the `register` function from `@teamhanko/hanko-elements` into your Solid component. Call it with the Hanko API URL as an argument to register `<hanko-auth>` with the browser's [CustomElementRegistry](https://developer.mozilla.org/de/docs/Web/API/CustomElementRegistry). Once done, include it in your JSX.
