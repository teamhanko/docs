...
3. **Find the matching JWK from the list of JWKS retrieved** The matching JWK is the one that has the same kid found at step 1.
...
<Warning>
  ...
  time a token needs to be validated. It is recommended to retrieve the JWK when
  no JWK matches the token kid has been found at (step 3).
</Warning>
