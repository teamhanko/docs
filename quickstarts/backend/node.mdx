...
<Note>See [here](/guides/sessions) for more details on sessions</Note>
...
<Tabs>
    <Tab title="Stateful (server-side)">
        1. **Validate the session using the `sessions/validate` [endpoint](/api-reference/public/session-management/validate-a-session-1):**
